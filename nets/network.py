...
class PoseEstimationNetwork(object):
    ...
    def EstimationNet(self, image, hand_side, evaluation):
        """ Full pipeline: HandSegNet + PoseNet + Pose3DNet.
        ...
        hand_scoremap = self.HandSegNet(image)
        ...
        keypoint_coord3d = self.Pose3DNet(keypoints_scoremap, hand_side, evaluation)
    ...
    def HandSegCrop(self, image):
        """ 
            All preparation before PoseNet: HandSegNet + Crop
        ...
        return hand_scoremap, image_crop, scale_crop, center
    
    @staticmethod
    def HandSegNet(image, train=False):
        """
    ...
    def PoseNet(self, image_crop, train=False):
        """ 
            PoseNet: Detects the 2D hand keypoints.
    ...
    def Pose3DNet(self, keypoints_scoremap, hand_side, evaluation, train=False):
        """ 
        ...
        scoremap_pooled = tf.nn.avg_pool(keypoints_scoremap, ksize=[1, 8, 8, 1], strides=[1, 8, 8, 1], padding='SAME')
        # infer coordinates in the canonical frame
        coord_can = self._inference_pose3d_can(scoremap_pooled, hand_side, evaluation, train=train)

        # infer viewpoint
        rot_mat = self._inference_viewpoint(scoremap_pooled, hand_side, evaluation, train=train)
    ...
    def _get_rot_mat(self, ux_b, uy_b, uz_b):
        ...
        with tf.name_scope('get_rot_mat'):
            ...
            trafo_matrix = self._stitch_vecs_to_matrix([ct+ux*ux*one_ct, ux*uy*one_ct-uz*st, ux*uz*one_ct+uy*st,
    ...
    @staticmethod
    def _stitch_vecs_to_matrix(vector_list):
